    ...
    public class CedarHostTests
    {
        ...
        public void When_execute_valid_command_then_should_not_throw()
        {
            using (var host = new CedarHost(new TestBootstrapper()))
            {
                ...
                {
                    Func<Task> act = () => client.ExecuteCommand(new TestCommand(), Guid.NewGuid());
                    act.ShouldNotThrow();
                }
            }
        }
        ...
        public void When_execute_command_without_handler_then_should_throw()
        {
            using (var host = new CedarHost(new TestBootstrapper()))
            {
                using (var client = host.CreateClient())
                {
                    Func<Task> act = () => client.ExecuteCommand(new TestCommandWithoutHandler(), Guid.NewGuid());
